...
def run_microgrid_model(battery_capacity, solar_panel_count):
    ...
    model.SellPrice = Param(model.T, initialize= 0.045) 
    ...
    def load_balance_rule(m,month, period):
        return (m.SolarUsed[month, period] + m.EnergyBought[month, period] + m.BatteryDischarge[month, period] == m.Load[month, period] + m.EnergySold[month, period] + m.BatteryCharge[month, period] + m.Dump[month, period])
  ...
  for (month, period) in model.T ), sense=minimize )
   ...
   #ssei 
    ...
    values = [ bdi, lcse, NECI, ssi, gdi, ELR , EGR , BLR, GLR]
    return ELR
...
df = pd.DataFrame(
    index=battery_capacities,
    columns=Panels_num,
    dtype=float
)

for battery_cap in battery_capacities:
    for panels in Panels_num:
        print(f"Running: Battery={battery_cap}, Panels={panels}")
        try:
            output = run_microgrid_model(battery_cap, panels)
            if output is None:
                print(" → Infeasible / Failed")
                df.loc[battery_cap, panels] = np.nan
                continue
            df.loc[battery_cap, panels] = output
        except Exception as e:
            print(f" → Error at B={battery_cap}, P={panels}")
            print(e)
            df.loc[battery_cap, panels] = np.nan
# Save results
df.to_csv("sensitivity_results_bdi.csv")
...
plt.title("Export over Load Ratio Heatmap")
plt.ylabel("Battery Capacity (kWh)")
plt.xlabel("Number of Solar Panels (0.58 kW panel)")
plt.tight_layout()
plt.show()
